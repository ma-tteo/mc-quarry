...
class APIClient:
    ...
    def get_json(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                r = self.session.get(
                    url, params=params, headers=current_headers, timeout=20
                )
                if r.status_code == 200:
                    ...
                    logger.debug(
                        f"API Error {r.status_code} for {url} (Attempt {attempt})"
                    )
    ...
    def get_latest_file_cf(
        ...
        data = self.get_json(
            url, params={"pageSize": 50}, headers=self.get_cf_headers()
        )
        if isinstance(data, dict) and "data" in data:
            files = data["data"]
            valid_files = []
            for f in files:
                if force_latest or mc_version in f.get("gameVersions", []):
                    if mod_loader_type != 0:
                        ...
                        target_loaders = []
                        if mod_loader_type == 4:
                            target_loaders = ["fabric", "quilt"]
                        elif mod_loader_type == 1:
                            target_loaders = ["forge"]
                        elif mod_loader_type == 6:
                            target_loaders = ["neoforge"]
                        elif mod_loader_type == 5:
                            target_loaders = ["quilt"]

                        if any(
                            v.lower() in target_loaders
