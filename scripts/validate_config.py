...
class ConfigValidator:
    ...
    def validate_mod_lists(self):
        ...
        mod_fields = ['core_mods', 'utility_mods', 'light_qol_mods',
            ...
            for rule in version_rules:
                if not isinstance(rule, str):
                    self.errors.append({
                        ...
                        'message': (
                            f"Rule for '{mod}' should be string, "
                            f"got {type(rule).__name__}: {rule}"
                        ),
                        'mod': mod,
                        'rule': rule
                    })
    ...
    async def deep_validate_mods(self):
        ...
        for field in ['core_mods', 'utility_mods', 'light_qol_mods',]:
        ...
        for _i, (field, mod) in enumerate(all_mods[:20], 1):  # Limit to 20 for speed
        ...
        if len(all_mods) > 20:
            self.info.append({
                ...
                'message': (
                    f"Only checked 20/{len(all_mods)} mods "
                    f"(use full analysis for complete check)"
                )
            })

        print(f"\n  {BColors.OKGREEN}✓ {valid_count} valid, "
              f"{invalid_count} not found{BColors.ENDC}")
    ...
    def print_report(self):
        ...
        # Errors
        if self.errors:
            _err_count = len(self.errors)
            _err_prefix = f"{BColors.FAIL}║{BColors.ENDC}  {BColors.BOLD}"
        ...
        # Warnings
        if self.warnings:
            _warn_count = len(self.warnings)
            _warn_prefix = f"{BColors.WARNING}║{BColors.ENDC}  {BColors.BOLD}"
            print(f"{BColors.WARNING}╔══════════════════════════════════════════════════════════════╗{BColors.ENDC}")
            print(f"{_warn_prefix}WARNINGS ({_warn_count}){BColors.ENDC}")
        ...
        # Info
        if self.info:
            _info_count = len(self.info)
            _info_prefix = f"{BColors.OKCYAN}║{BColors.ENDC}  {BColors.BOLD}"
            print(f"{BColors.OKCYAN}╔══════════════════════════════════════════════════════════════╗{BColors.ENDC}")
            print(f"{_info_prefix}INFO ({_info_count}){BColors.ENDC}")
...
def main():
    ...
    parser.add_argument(
        ...
        help='Enable deep validation (checks mods against API)'
    )
