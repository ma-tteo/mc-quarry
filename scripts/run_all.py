...
Usage:
    python3 scripts/run_all.py [--config PATH] [--modpack-dir PATH]
    [--api-key KEY] [--mc-version VERSION]
"""
import json
import subprocess
import sys
...
class AnalysisRunner:
    ...
    def run_script(self, script: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Print summary
        if result['success']:
            status = f"{BColors.OKGREEN}✓ SUCCESS{BColors.ENDC}"
        else:
            status = f"{BColors.FAIL}✗ FAILED{BColors.ENDC}"
    ...
    def generate_summary(self):
        ...
        for result in self.results:
            ...
            print(f"{result['name']:<25} {status_color}{status}"
                  f"{BColors.ENDC:<15} {result['duration']:.1f}s")
        ...
        if critical_fails > 0:
            print(f"\n{BColors.FAIL}⚠ CRITICAL FAILURES DETECTED"
                  f" - Review reports above{BColors.ENDC}\n")
            sys.exit(1)
        elif fail_count > 0:
            print(f"\n{BColors.WARNING}⚠ Some scripts failed"
                  f" - Review reports above{BColors.ENDC}\n")
            sys.exit(2)
        else:
