...
class ConflictAnalyzer:
        ...
        for subdir in self.modpack_dir.iterdir():
            ...
            for info_file in subdir.glob("*.modinfo"):
                try:
                    ...
                    print(f"{BColors.WARNING}⚠️ Error reading mod info {info_file}: "
                          f"{e}{BColors.ENDC}")
    ...
    def check_known_conflicts(self):
        ...
        installed_lower = {slug.lower(): slug for slug in self.installed_mods}
    ...
    def check_duplicate_functionality(self):
        ...
        if count > 0:
            print(f"  {BColors.WARNING}⚠ Found {count} duplicate functionality "
                  f"groups{BColors.ENDC}")
        else:
    ...
    def generate_recommendations(self):
        ...
        for violation in self.results['incompatibility_violations']:
            self.results['recommendations'].append({
                ...
                'priority': 'high',
                'message': (
                    f"{violation['mod']} is incompatible with MC "
                    f"{violation['mc_version']} ({violation['rule']})"
                ),
            })
    ...
    def print_report(self):
        ...
        if self.results['incompatibility_violations']:
            _inv_count = len(self.results['incompatibility_violations'])
            _inv_prefix = f"{BColors.FAIL}║{BColors.ENDC}  {BColors.BOLD}"
            print(f"{BColors.FAIL}╔══════════════════════════════════════════════════════════════╗{BColors.ENDC}")
            print(f"{_inv_prefix}INCOMPATIBILITY VIOLATIONS ({_inv_count}){BColors.ENDC}")
        ...
        if self.results['known_conflicts']:
            _kc_count = len(self.results['known_conflicts'])
            _kc_prefix = f"{BColors.FAIL}║{BColors.ENDC}  {BColors.BOLD}"
            print(f"{BColors.FAIL}╔══════════════════════════════════════════════════════════════╗{BColors.ENDC}")
            print(f"{_kc_prefix}KNOWN CONFLICTS ({_kc_count}){BColors.ENDC}")
        ...
        if self.results['duplicate_functionality']:
            _dup_count = len(self.results['duplicate_functionality'])
            _dup_prefix = f"{BColors.WARNING}║{BColors.ENDC}  {BColors.BOLD}"
            print(f"{BColors.WARNING}╔══════════════════════════════════════════════════════════════╗{BColors.ENDC}")
            print(f"{_dup_prefix}DUPLICATE FUNCTIONALITY ({_dup_count}){BColors.ENDC}")
            for d in self.results['duplicate_functionality']:
                _mods_str = ', '.join(d['mods'])
                print(f"{BColors.WARNING}║{BColors.ENDC}    ⚠ {d['category'].title()}: {_mods_str}")
                print(f"{BColors.WARNING}║{BColors.ENDC}      {d['recommendation']}")
            print(f"{BColors.WARNING}╚══════════════════════════════════════════════════════════════╝{BColors.ENDC}\n")

        # Recommendations
        if self.results['recommendations']:
            _rec_count = len(self.results['recommendations'])
            _rec_prefix = f"{BColors.OKCYAN}║{BColors.ENDC}  {BColors.BOLD}"
            print(f"{BColors.OKCYAN}╔══════════════════════════════════════════════════════════════╗{BColors.ENDC}")
            print(f"{_rec_prefix}RECOMMENDATIONS ({_rec_count}){BColors.ENDC}")
            for i, rec in enumerate(self.results['recommendations'], 1):
                priority_color = BColors.FAIL if rec['priority'] == 'high' else BColors.WARNING
                _priority_upper = rec['priority'].upper()
                print(f"{BColors.OKCYAN}║{BColors.ENDC}    {i}. {priority_color}[{_priority_upper}]"
                      f"{BColors.ENDC} {rec['message']}")
            print(f"{BColors.OKCYAN}╚══════════════════════════════════════════════════════════════╝{BColors.ENDC}\n")
